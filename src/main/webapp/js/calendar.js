...
function writeIframe()
{
    ...
    "    <table id=tableDay height=120 width=140 border=0 cellspacing=1 cellpadding=0>";
         ...
         WebCalendar.btnBgColor +";cursor: hand;   border: 0' onfocus='this.blur()'"+
         " type=button value='清除' onclick=\"parent.returnDate('')\"></td></tr></table>"+
...
function calendar() //主调函数
{
    ...
    try{
    	var txtDate=WebCalendar.objExport.value.trim();
    	if(txtDate.length=6){
    		txtDate=txtDate.substr(0,4)+"-"+txtDate.substr(5,2);
    	}
        if (txtDate.trim() != ""){
            WebCalendar.dateStyle = txtDate.trim().match(WebCalendar.dateReg);
...
function returnDate() //根据日期格式等返回用户选定的日期
{
    if(WebCalendar.objExport)
    {
        ...
        if( arguments[0] == "" ) returnValue = "" ;
       	var pattern = /-/g;
	   	var returnValue=returnValue.replace(pattern, '');
