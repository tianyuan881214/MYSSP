...
<html>
	...
	<body>
		<fieldset>
			<legend><%=OperationLoginfo.TABLE_MER_ALIAS%>信息</legend>
			<mvc:form action="list.do" theme="simple"  method="post" commandName="model">
			<table class="grid">
				<tr>
					<td class="lgridlist"><%=OperationLoginfo.ALIAS_MER_NO%></td>	
					<td>${model.merNo}</td>
					
		           <td class="lgridlist"><%=OperationLoginfo.ALIAS_MER_NAME%></td>	
					<td>${model.merName}</td>
		        </tr>
		        <tr>
			...
			<table class="b">
			<tr  align="center">
				<td><input type="button" value="返回" onclick="window.location='merList.do?<mytag:params includes="s_*" type="queryStringUtf"/>'"/></td>
			</tr>
		</table>
	</mvc:form>
	</fieldset>	
