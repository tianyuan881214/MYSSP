...
import com.yada.ssp.query.AlipayTranLsQuery;
import com.yada.ssp.service.AlipayTranLsManager;
import com.yada.ssp.service.AlipayTranLsManager;
...
/**
 * Created by Administrator on 2017/9/9.AlipayTranLs alia
 */
@Controller
public class AlipayTranLsController extends BaseController {
    @Autowired
    private AlipayTranLsManager alipayTranLsDao;
    @Autowired
    private OrgManager orgManager;

    @RequestMapping
    public String list(Model model, AlipayTranLsQuery query) {
        //关于权限控制的问题，在orgId;
        ...
        Page page = alipayTranLsDao.queryPage(query);
        model.addAttribute("query", query);
        model.addAttribute("page", page);
        return "/ssp_pages/AlipayTranLs/list";
    }
     ...
     */
    @RequestMapping
    public ResponseEntity<byte[]> downloadWebCheckFileReport(AlipayTranLsQuery query) throws IOException, org.apache.poi.openxml4j.exceptions.InvalidFormatException, ParseException {
        String orgId = getCurUser().getUserExt().getOrgId();
        query.setOrgId(orgId);

        Page page1 = alipayTranLsDao.queryPage(query);
        List<AlipayTranLs> webChatTranLs = page1.getResult();
        List<Map<String, String>> page = new ArrayList<>();
        for (AlipayTranLs s : webChatTranLs) {
            s.setState(s.getState().equals("0")?"已复核":"未复核");
        ...
        String reportName = "支付宝流水报表_" + date.getStringToDate() + "_" + getCurUser().getLoginName();
        String[] reportLine = {"merchantId","outTradeNo", "batchNO",
                "merchantCname", "state", "operatorId"
                ,"refundFlag","totalAmount","traceNo",
                "tranType","secondName","firstName","oldTrandate"};
        return createExcel(reportName, reportLine, ReportHeader.alipayTranLsHeader, page);
    }
