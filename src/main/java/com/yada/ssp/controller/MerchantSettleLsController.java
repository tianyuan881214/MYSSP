...
import com.yada.ssp.query.MerchantSettleLsQuery;
import com.yada.ssp.service.MerchantSettleLsManager;
 ...
 */
@Controller
public class MerchantSettleLsController extends BaseController {
    @Autowired
    private MerchantSettleLsManager merchantSettlelsManager;
    @Autowired
    private OrgManager orgManager;

    @RequestMapping
    public String list(Model model, MerchantSettleLsQuery query) {
        //关于权限控制的问题，在orgId;
        ...
        Page page = merchantSettlelsManager.queryPage(query);
        model.addAttribute("page", page);
        model.addAttribute("query", query);
        return "/ssp_pages/MerchantSettleLs/list";
    }
     ...
     */
    @RequestMapping
    public ResponseEntity<byte[]> downloadWebCheckFileReport(MerchantSettleLsQuery query) throws IOException, org.apache.poi.openxml4j.exceptions.InvalidFormatException, ParseException {

            String orgId = getCurUser().getUserExt().getOrgId();
            query.setOrgId(orgId);


        Page page1 = merchantSettlelsManager.queryPage(query);
        List<MerchantSettleLs> merchantSettleLs = page1.getResult();
        List<Map<String, String>> page = new ArrayList<>();
        for (MerchantSettleLs s : merchantSettleLs) {
           /* merchantSettleSumLs.setBusinessBase(merchantSettleSumLs.getBusinessBase().equals("000000000000002")?"支付宝业务":"微信业务");*/

            page.add(BeanUtils.beanToMap(s));
        }
        StringUtils date = new StringUtils();
        String reportName = "商户汇总报表_" + date.getStringToDate() + "_" + getCurUser().getLoginName();
        String[] reportLine = {
                "merchantId","merchantCname","count","aliWeTotal",
                "aliWeFee","settleFee","aliTranAmt","aliFee",
                "weTranAmt","wechatFee","othFee","myFee",
                "gsFee","firstNo","firstName","secondNo",
                "secondName","accountNo","accountName","openingBank",
                "openingBankNo",

        };
        return createExcel(reportName, reportLine, ReportHeader.merchantSettleLsReportHeader, page);
    }
