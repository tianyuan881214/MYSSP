 ...
 */
@Controller
public class PayTranWeChatLsController extends BaseController {
    @Autowired
    private PayTranWeChatLsManager payTranWeChatLsManager;
    @Autowired
    private OrgManager orgManager;

    @RequestMapping
    public String list(Model model, PayTranWechatLsQuery query) {
...
*/
        Page page = payTranWeChatLsManager.queryPage(query);
        model.addAttribute("page", page);
        model.addAttribute("query",query);
        return "/ssp_pages/PayTranWeChatLs/list";
    }

    @RequestMapping
    public String show(Model model, String id) {
        PayTranWeChatLs payTranWeChatLs = payTranWeChatLsManager.getById(id);
        model.addAttribute("model",payTranWeChatLs);
        return "/ssp_pages/PayTranWeChatLs/show";
    }
     ...
     */
    @RequestMapping
    public ResponseEntity<byte[]> downloadPayTranWechatLsReport(PayTranWechatLsReportQuery query) throws IOException, InvalidFormatException, ParseException {
        String orgId = getCurUser().getUserExt().getOrgId();
        Org org = orgManager.getById(orgId);
        if ("2".equals(org.getOrgLev().toString())) {
            query.setOrgId(orgId);
        }
        List<PayTranWeChatLs> payTranAlipayLses = payTranWeChatLsManager.findAll(query);
        List<Map<String, String>> page = new ArrayList<>();
        for (PayTranWeChatLs payTranAlipayLs : payTranAlipayLses) {
            page.add(BeanUtils.beanToMap(payTranAlipayLs));
        }
        StringUtils date = new StringUtils();
        String reportName = "微信流水_" + date.getStringToDate() + "_" + getCurUser().getLoginName();
        String[] reportLine = {"merchantNo", "merchantCname", "outTradeNo", "transactionId", "totalAmount", "tradeType","resultCode", "tranDate", "tranTime", "name"};
        return createExcel(reportName, reportLine, ReportHeader.payTranWechatLsHeader, page);
    }
