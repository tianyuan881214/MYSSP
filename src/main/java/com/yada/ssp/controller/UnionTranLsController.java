...
import com.yada.ssp.model.UnionTranLs;
import com.yada.ssp.query.UnionTranLsQuery;
import com.yada.ssp.service.UnionTranLsManager;
...
/**
 * Created by Administrator on 2017/9/9.UnionTranLs alipaytranls
 */
@Controller
public class UnionTranLsController extends BaseController {
    @Autowired
    private UnionTranLsManager unionTranLsManager;
    @Autowired
    private OrgManager orgManager;

    @RequestMapping
    public String list(Model model, UnionTranLsQuery query) {
        //关于权限控制的问题，在orgId;
        ...
        Page page = unionTranLsManager.queryPage(query);
        model.addAttribute("query", query);
        model.addAttribute("page", page);
        return "/ssp_pages/UnionTranLs/list";
    }
     ...
     */
    @RequestMapping
    public ResponseEntity<byte[]> downloadWebCheckFileReport(UnionTranLsQuery query) throws IOException, org.apache.poi.openxml4j.exceptions.InvalidFormatException, ParseException {
        String orgId = getCurUser().getUserExt().getOrgId();
        query.setOrgId(orgId);

        Page page1 = unionTranLsManager.queryPage(query);
        List<UnionTranLs> unionTranLs = page1.getResult();
        List<Map<String, String>> page = new ArrayList<>();
        for (UnionTranLs s : unionTranLs) {
            s.setState(s.getState().equals("0")?"已入驻":"未入驻");
        ...
        String reportName = "银联流水报表_" + date.getStringToDate() + "_" + getCurUser().getLoginName();
        String[] reportLine = {"merId","orderId", "merchantCname",
        "queryId", "accNo", "txNamt"
                ,"secondName","firstName","state",
                "traceTime"};
        return createExcel(reportName, reportLine, ReportHeader.unionTranLsHeader, page);
    }
