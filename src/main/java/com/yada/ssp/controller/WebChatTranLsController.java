...
import com.yada.ssp.query.WebChatTranLsQuery;
import com.yada.ssp.service.AlipaySettleleLsManager;
import com.yada.ssp.service.WebChatTranLsManager;
...
/**
 * Created by Administrator on 2017/9/9. WebChatTranLs webchattranls
 */
@Controller
public class WebChatTranLsController extends BaseController {
    @Autowired
    private WebChatTranLsManager webChatTranLsManager;
    @Autowired
    private OrgManager orgManager;

    @RequestMapping
    public String list(Model model, WebChatTranLsQuery query) {
        //关于权限控制的问题，在orgId;
        ...
        Page page = webChatTranLsManager.queryPage(query);
        model.addAttribute("query", query);
        model.addAttribute("page", page);
        return "/ssp_pages/WebChatTranLs/list";
    }
     ...
     */
    @RequestMapping
    public ResponseEntity<byte[]> downloadWebCheckFileReport(WebChatTranLsQuery query) throws IOException, org.apache.poi.openxml4j.exceptions.InvalidFormatException, ParseException {

            String orgId = getCurUser().getUserExt().getOrgId();
            query.setOrgId(orgId);


        Page page1 = webChatTranLsManager.queryPage(query);
        List<WebChatTranLs> webChatTranLs = page1.getResult();
        List<Map<String, String>> page = new ArrayList<>();
        for (WebChatTranLs s : webChatTranLs) {
        ...
        String reportName = "微信流水报表_" + date.getStringToDate() + "_" + getCurUser().getLoginName();
        String[] reportLine = {"merchantNo","outTradeNo", "authCode",
                "batchNO", "merchantCname", "state"
                ,"operationId","refundFlag","returnCode",
                "totalAmount","traceNo","tranType","transactionId","secondName",
                "firstName","oldTranDate"};
        return createExcel(reportName, reportLine, ReportHeader.webChatTranLsHeader, page);
    }
