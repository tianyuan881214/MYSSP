...
import com.yada.ssp.query.WebChatSettleLsQuery;
import com.yada.ssp.service.WebChatSettlelsManager;
 ...
 */
@Controller
public class WebChatSettleLsController extends BaseController {
    @Autowired
    private WebChatSettlelsManager webChatSettlelsManager;
    @Autowired
    private OrgManager orgManager;

    @RequestMapping
    public String list(Model model, WebChatSettleLsQuery query) {
        //关于权限控制的问题，在orgId;
        ...
        Page page = webChatSettlelsManager.queryPage(query);
        model.addAttribute("page", page);
        model.addAttribute("query", query);
        return "/ssp_pages/WebChatSettleLs/list";
    }
     ...
     */
    @RequestMapping
    public ResponseEntity<byte[]> downloadWebCheckFileReport(WebChatSettleLsQuery query) throws IOException, org.apache.poi.openxml4j.exceptions.InvalidFormatException, ParseException {
        if(query.getOrgId() ==null||query.getOrgId().equals("")) {
            String orgId = getCurUser().getUserExt().getOrgId();
            query.setOrgId(orgId);
        }

        Page page1 = webChatSettlelsManager.queryPage(query);
        List<WebChatSettleLs> WebChatSettleLs = page1.getResult();
        List<Map<String, String>> page = new ArrayList<>();
        for (WebChatSettleLs s : WebChatSettleLs) {
        ...
        String reportName = "微信清算报表_" + date.getStringToDate() + "_" + getCurUser().getLoginName();
        String[] reportLine = {
                "merchantId","merchantCname","subMerchantId","outTradeNo",
                "firsetName","secondName","tranStart","totalAmount",
                "amountFee","settleFee","webFee","feeRate",
                "myFee","othFee","gsFee","accountType",
                "tranDate","settleDate"
        };
        return createExcel(reportName, reportLine, ReportHeader.WebChatSettlsHeader, page);
    }
