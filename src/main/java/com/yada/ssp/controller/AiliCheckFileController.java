...
import com.yada.ssp.service.AiliCheckFileManager;
import com.yada.xmlUtils.DateUtil;
 ...
 */
@Controller
public class AiliCheckFileController extends BaseController {
    @Autowired
    private AiliCheckFileManager ailiCheckFileManager;
    @Autowired
    private OrgManager orgManager;

    @RequestMapping
    public String list(Model model, AiliCheckFileQuery query) {
        ...
        Page page = ailiCheckFileManager.queryPage(query);
        model.addAttribute("page", page);
        model.addAttribute("query", query);
        return "/ssp_pages/AiliCheckFile/list";
    }

    @RequestMapping
    public String show(@RequestParam("tradeNo") String tradeNo,
                       @RequestParam("tranType") String tranType,
                       @RequestParam("createDate") String createDate,
                       @RequestParam("finishDate") String finishDate,
                       Model model) {
        AiliCheckFileQuery Query = new AiliCheckFileQuery();
        Query.setCreateDate(createDate);
        Query.setTradeNo(tradeNo);
        Query.setFinishDate(finishDate);
        AiliCheckFile ailiCheckFile1 = ailiCheckFileManager.findByTradeNoAndTranTypeAndCreateDate(Query);
        model.addAttribute("model", ailiCheckFile1);
        return "/ssp_pages/AiliCheckFile/show";
    }
     ...
     */
    @RequestMapping
    public ResponseEntity<byte[]> downloadCheckFileTranLsReport(AiliCheckFileQuery query) throws IOException, InvalidFormatException, ParseException {
        String orgId = getCurUser().getUserExt().getOrgId();
        Org org = orgManager.getById(orgId);
        if ("2".equals(org.getOrgLev().toString())) {
            query.setOrgId(orgId);
        }
        List<AiliCheckFile> ailiCheckFilels = ailiCheckFileManager.findAll(query);
        List<Map<String, String>> page = new ArrayList<>();
        for (AiliCheckFile AiliCheckFile : ailiCheckFilels) {
            page.add(BeanUtils.beanToMap(AiliCheckFile));
        }
        ...
        return createExcel(reportName, reportLine, ReportHeader.ailiCheckFileHeader, page);
